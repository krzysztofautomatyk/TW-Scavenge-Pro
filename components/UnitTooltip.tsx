...
const UnitTooltip: React.FC<Props> = ({ unit, position }) => {
  ...
  const translatedUnit = t.units[unit.id];
  ...
  return (
    ...
    >
      ...
      <div className="p-1 border-b flex justify-between items-center transition-colors
        ...
        <span className="font-bold text-sm px-1">{translatedUnit.name}</span>
      ...
      <div className="p-2 relative">
        ...
        <div className="flex gap-2">
            <div className="flex-1 space-y-3">
                ...
                <div>
                    ...
                    <div className="flex gap-2 items-center text-[11px] dark:text-slate-300">
                        <span className="flex items-center gap-1"><div className="w-3 h-3 bg-[#a06840] rounded-full border border-black/20" title="Wood"></div> {unit.stats.wood}</span>
                        <span className="flex items-center gap-1"><div className="w-3 h-3 bg-[#e88840] rounded-full border border-black/20" title="Clay"></div> {unit.stats.clay}</span>
                        <span className="flex items-center gap-1"><div className="w-3 h-3 bg-[#888899] rounded-full border border-black/20" title="Iron"></div> {unit.stats.iron}</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-1 text-center">
                    <div className="px-0.5 py-0.5 border transition-colors
                        ...
                        <div className="font-bold text-[10px] text-[#5c2e08] dark:text-slate-400">{t.unitStats.pop}</div>
                        <div className="dark:text-slate-200">{unit.stats.pop}</div>
                    </div>
                    <div className="px-0.5 py-0.5 border transition-colors
                                    bg-[#e7d8af] border-[#c5b58d]
                                    dark:bg-slate-800 dark:border-slate-700">
                        <div className="font-bold text-[10px] text-[#5c2e08] dark:text-slate-400">{t.unitStats.speed}</div>
                    ...
                    <div className="px-0.5 py-0.5 border transition-colors relative overflow-hidden
                        ...
                        <div className="font-black text-[10px] text-[#402000] dark:text-brand-300 uppercase tracking-wide scale-95 transform">{t.unitStats.carry}</div>
            ...
            <div className="w-24 shrink-0 flex items-start justify-center">
                 <img src={unit.image} alt={translatedUnit.name} className="max-w-full h-auto drop-shadow-md" />
            </div>
        </div>
                ...
                <tbody>
                    <tr className="border-b transition-colors border-[#dfcf9e] dark:border-slate-700">
                        <td className="p-1 flex items-center gap-1 dark:text-slate-300">
                             <Swords size={12} className="text-black dark:text-slate-400" /> {t.unitStats.attack}
                        </td>
                    ...
                    <tr className="border-b transition-colors 
                        ...
                        <td className="p-1 flex items-center gap-1 dark:text-slate-300">
                             <Shield size={12} className="text-[#1a3875] dark:text-blue-400" /> {t.unitStats.defGen}
                        </td>
                        <td className="p-1 text-right font-bold dark:text-slate-200">{unit.stats.defense}</td>
                    </tr>
                    <tr className="border-b transition-colors border-[#dfcf9e] dark:border-slate-700">
                        <td className="p-1 flex items-center gap-1 dark:text-slate-300">
                             <Shield size={12} className="text-[#5c3e1c] dark:text-amber-500" /> {t.unitStats.defCav}
                        </td>
                    ...
                    <tr className="transition-colors
                        ...
                        <td className="p-1 flex items-center gap-1 dark:text-slate-300">
                             <Shield size={12} className="text-[#2d5c1c] dark:text-emerald-500" /> {t.unitStats.defArch}
                        </td>
